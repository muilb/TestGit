...
// //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// We know that you love 'free', we love it too :)! And so our entire software
// stack is free and even Open Source!
// Our routing service is also free for certain applications or smaller volume.
// Be fair, grab an API key and support us:
// https://graphhopper.com/#directions-api Misuse of API keys that you don't own
// is prohibited and you'll be blocked.
// //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
if (!host) {
	if (location.port === '') {
				...
				function(e) {
					...
					if (History.enabled) {
						History.Adapter.bind(window, 'statechange', function() {
							...
							// Chrome and Safari always emit a popstate event on
							// page load, but Firefox doesn’t
					...
					$
							.when(
									ghRequest
											.fetchTranslationMap(urlParams.locale),
									ghRequest.getInfo())
							.then(
									function(arg1, arg2) {
										// init translation retrieved from first
										// call (fetchTranslationMap)
										var translations = arg1[0];

										// init language
										// 1. determined by Accept-Language
										// header, falls back to 'en' if no
										// translation map available
										// 2. can be overwritten by url
										// parameter
										ghRequest
										...
										function createButton(vehicle) {
											var button = $("<button class='vehicle-btn' title='"
													+ tr(vehicle) + "'/>");
											button.attr('id', vehicle);
											button.html("<img src='img/"
													+ vehicle + ".png' alt='"
													+ tr(vehicle) + "'></img>");
										...
										if (json.features) {
											...
											if (vehicles.length > 0)
												ghRequest
														.initVehicle(vehicles[0]);

											for ( var key in json.features) {
												vehiclesDiv
														.append(createButton(key
																.toLowerCase()));
											}
										}
									...
									function(err) {
										log(err);
										$('#error').html(
												'GraphHopper API offline? <a href="http://graphhopper.com/maps">Refresh</a>'
														+ '<br/>Status: '
														+ err.statusText
														+ '<br/>' + host);
...
function initFromParams(params, doQuery) {
	...
	var fromAndTo = params.from && params.to, routeNow = params.point
			&& params.point.length >= 2 || fromAndTo;
...
function resolveCoords(pointsAsStr, doQuery) {
	...
	if (ghRequest.route.isResolved()) {
		...
		// at least one text input from user -> wait for resolve as we need the
		// coord for routing
...
function checkInput() {
	var template = $('#pointTemplate').html(), len = ghRequest.route.size();
	...
	for (var i = 0; i < len; i++) {
		...
		if (div.length === 0) {
			$('#locationpoints > div.pointAdd').before(nanoTemplate(template, {
				id : i
			}));
		...
		div.data("index", i);
		div.find(".pointFlag").attr(
				"src",
				(toFrom === FROM) ? 'img/marker-small-green.png'
						: ((toFrom === TO) ? 'img/marker-small-red.png'
								: 'img/marker-small-blue.png'));
		if (len > 2) {
...
function adjustMapSize() {
	...
	$("#input").height(height);
	// reduce info size depending on how heigh the input_header is and reserve
	// space for footer
...
function initMap(selectLayer) {
	...
	var lyrk = L.tileLayer('https://tiles.lyrk.org/' + tp
			+ '/{z}/{x}/{y}?apikey=6e8cfef737a140e2a58c8122aaa26077', {
		attribution : osmAttr
				+ ', <a href="https://geodienste.lyrk.de/">Lyrk</a>',
		subdomains : [ 'a', 'b', 'c' ]
	});

	var omniscale = L.tileLayer
			.wms(
					'https://maps.omniscale.net/v1/mapsgraph-bf48cc0b/tile',
					{
						layers : 'osm',
						attribution : osmAttr
								+ ', &copy; <a href="http://maps.omniscale.com/">Omniscale</a>'
					});
	...
	var openMapSurfer = L
			.tileLayer(
					...
					{
						attribution : osmAttr
								+ ', <a href="http://openmapsurfer.uni-hd.de/contact.html">GIScience Heidelberg</a>'
					});

	// not an option as too fast over limit
	// var mapbox=
	// L.tileLayer('https://{s}.tiles.mapbox.com/v4/peterk.map-vkt0kusv/{z}/{x}/{y}.png?access_token=pk.eyJ1IjoicGV0ZXJrIiwiYSI6IkdFc2FJd2MifQ.YUd7dS_gOpT3xrQnB8_K-w',
	// {
	// attribution: osmAttr + ', <a
	// href="https://www.mapbox.com/about/maps/">&copy; MapBox</a>'
	// });
	...
	var sorbianLang = L
			.tileLayer(
					...
					{
						attribution : osmAttr
								+ ', <a href="http://www.alberding.eu/">&copy; Alberding GmbH, CC-BY-SA</a>'
					});

	var thunderTransport = L
			.tileLayer(
					'http://{s}.tile.thunderforest.com/transport/{z}/{x}/{y}.png',
					{
						attribution : osmAttr
								+ ', <a href="http://www.thunderforest.com/transport/" target="_blank">Thunderforest Transport</a>',
						subdomains : [ 'a', 'b', 'c' ]
					});
	...
	var thunderCycle = L
			.tileLayer(
					...
					{
						attribution : osmAttr
								+ ', <a href="http://www.thunderforest.com/opencyclemap/" target="_blank">Thunderforest Cycle</a>',
						subdomains : [ 'a', 'b', 'c' ]
					});

	var thunderOutdoors = L
			.tileLayer(
					'http://{s}.tile.thunderforest.com/outdoors/{z}/{x}/{y}.png',
					{
						attribution : osmAttr
								+ ', <a href="http://www.thunderforest.com/outdoors/" target="_blank">Thunderforest Outdoors</a>',
						subdomains : [ 'a', 'b', 'c' ]
					});
	...
	var wrk = L
			.tileLayer(
					...
					{
						attribution : osmAttr
								+ ', <a href="http://wanderreitkarte.de" target="_blank">WanderReitKarte</a>',
	...
	L.control.layers(baseMaps, {
		'Traffic Flow' : MQ.trafficLayer({
			layers : [ 'flow' ]
		}),
		'Traffic Incidents' : MQ.trafficLayer({
			layers : [ 'incidents' ]
		})
	}).addTo(map);
	...
	// end lbmui
	/*
	 * routingLayer.options = {style: {color: "#1F40C4", "weight": 5, "opacity":
	 * 0.6}, onEachFeature: function (feature, layer) { layer.on('contextmenu',
	 * function (e) { alert('The GeoJSON layer has been clicked'); }); }}; //
	 * route color and style
	 */
}

function setToStart(e) {
	var latlng = e.target.getLatLng(), index = ghRequest.route
			.getIndexByCoord(latlng);
...
function setFlag(coord, index) {
	if (coord.lat) {
		var toFrom = getToFrom(index), marker = L.marker(
				[ coord.lat, coord.lng ],
				{
					icon : ((toFrom === FROM) ? iconFrom
							: ((toFrom === TO) ? iconTo : iconInt)),
					draggable : true,
					contextmenu : true,
					contextmenuItems : [
							{
							...
							},
							{
								...
								index : 3,
								state : 2,
								disabled : (toFrom !== -1 && ghRequest.route
										.size() === 2) ? true : false
							// prevent to and from
							}, {
								separator : true,
				...
				}).addTo(routingLayer).bindPopup(
				((toFrom === FROM) ? 'Start' : ((toFrom === TO) ? 'End'
						: 'Intermediate ' + index)));
		// intercept openPopup
		marker._openPopup = marker.openPopup;
		marker.openPopup = function() {
			var latlng = this.getLatLng(), locCoord = ghRequest.route
					.getIndexFromCoord(latlng), content;
			if (locCoord.resolvedList && locCoord.resolvedList[0]
					&& locCoord.resolvedList[0].locationDetails) {
				var address = locCoord.resolvedList[0].locationDetails;
				content = ((address.road) ? address.road + ', ' : '')
		...
		if (toFrom === -1)
			marker.options.contextmenuItems.push(_tempItem);// because the
															// Mixin.ContextMenu
															// isn't initialized
...
/**
 * Returns a defer object containing the location pointing to a resolvedList
 * with all the found coordinates.
 */
	...
	if (locCoord.isResolved()) {
		...
		return $
				...
				.then(
						...
						function(err) {
							log("[nominatim_reverse] Error while looking up coordinate lat="
									+ locCoord.lat + "&lon=" + locCoord.lng);
		...
		return doGeoCoding(locCoord.input, 10, timeout).then(
				function(jsonArgs) {
					...
					for ( var index in jsonArgs) {
						...
						// de-duplicate via ignoring boundary stuff => not
						// perfect as 'Freiberg' would no longer be correct
						// if (json.type === "administrative")
						// continue;

						// if no different properties => skip!
						if (address
								&& JSON.stringify(address) === JSON
										.stringify(json.address))
							continue;
...
function formatLocationEntry(address) {
	...
	if (address.city || address.suburb || address.town || address.village
...
function doGeoCoding(input, limit, timeout) {
	...
	if (!limit)
		limit = 10;
	var url = nominatimURL + "?format=json&addressdetails=1&q="
			+ encodeURIComponent(input) + "&limit=" + limit;
	if (bounds.initialized) {
		// minLon, minLat, maxLon, maxLat => left, top, right, bottom
		url += "&bounded=1&viewbox=" + bounds.minLon + "," + bounds.maxLat
				+ "," + bounds.maxLon + "," + bounds.minLat;
	}
	...
	return $.ajax({
		...
		timeout : timeout
			...
			createCallback("[nominatim] Problem while looking up location "
					+ input));
}
...
function routeLatLng(request, doQuery) {
	// do_zoom should not show up in the URL but in the request object to avoid
	...
	request
			.doRequest(
					...
					function(json) {
						...
						if (request.hasElevation()) {
							if (elevationControl === null) {
								elevationControl = L.control.elevation({
									...
									useHeightIndicator : true, // if false a
																// marker is
																// drawn at map
																// position
									...
									hoverNumber : {
										decimalsX : 3, // decimals on distance
														// (always in km)
										decimalsY : 0, // deciamls on height
														// (always in m)
										formatter : undefined
									// custom formatter function may be injected
									},
									xTicks : undefined, // number of ticks in x
														// axis, calculated by
														// default according to
														// width
									yTicks : undefined, // number of ticks on y
														// axis, calculated by
														// default according to
														// height
									collapsed : false
								// collapsed mode, show chart on click or
								// mouseover
								});
						...
						descriptionDiv.append(tr("routeInfo", [ tmpDist,
								tmpTime ]));
						...
						if (path.instructions) {
							...
							for (var m = 0; m < len; m++) {
								...
								addInstruction(instructionsElement, instr, m,
										lngLat);
							}
							$("#info").append(instructionsElement);

							if (partialInstr) {
								var moreDiv = $("<button id='moreButton'>"
										+ tr("moreButton") + "..</button>");
								moreDiv
										.click(function() {
											moreDiv.remove();
							...
							osmRouteLink
									.attr(
											...
											"http://www.openstreetmap.org/directions?engine=graphhopper_"
													+ osmVehicle
													+ "&route="
													+ encodeURIComponent(request.from.lat
															+ ","
															+ request.from.lng
															+ ";"
															+ request.to.lat
							...
							osrmLink.attr("href",
									"http://map.project-osrm.org/?loc="
											+ request.from + "&loc="
											+ request.to);
							hiddenDiv
									.append("<br/><span>Compare with: </span>");
							...
							if (request.getVehicle().toUpperCase() === "FOOT") {
								...
								addToBing = "&mode=W";
							} else if ((request.getVehicle().toUpperCase()
									.indexOf("BIKE") >= 0)
							...
							googleLink.attr("href",
									"https://maps.google.com/?saddr="
											+ request.from + "&daddr="
											+ request.to + addToGoogle);
							hiddenDiv.append(googleLink);
							var bingLink = $("<a>Bing</a> ");
							bingLink.attr("href",
									"https://www.bing.com/maps/default.aspx?rtp=adr."
											+ request.from + "~adr."
											+ request.to + addToBing);
...
function addInstruction(main, instr, instrIndex, lngLat) {
	...
	if (lngLat) {
		instructionDiv.click(function() {
			...
			routeSegmentPopup = L.popup().setLatLng([ lngLat[1], lngLat[0] ])
					.setContent(title).openOn(map);
		});
	}
...
function parseUrl(query) {
	...
	for (var i = 0; i < vars.length; i++) {
		...
		if (typeof res[key] === "undefined" && key !== "heading"
				&& key !== "point") {
			if (value === 'true')
				value = true;
			else if (value === 'false')
...
function mySubmit() {
	var fromStr, toStr, viaStr, allStr = [], inputOk = true;
...
function stringFormat(str, args) {
	...
	if (str.indexOf("%1$s") >= 0) {
		...
		return str.replace(/\%(\d+)\$s/g, function(match, matchingNum) {
			matchingNum--;
			return typeof args[matchingNum] !== 'undefined' ? args[matchingNum]
					: match;
		});
	} else {
		...
		return str
				.replace(
						/\%s/g,
						function(match) {
							var val = typeof args[matchingNum] !== 'undefined' ? args[matchingNum]
									: match;
							matchingNum++;
							return val;
						});
...
function setAutoCompleteList(index) {
	...
	var options = {
		...
		/*
		 * as we use can potentially use jsonp we need to set the timeout to a
		 * small value
		 */
		timeout : 1000,
		...
		onSearchError : function(element, q, jqXHR, textStatus, errorThrown) {
			// too many errors if interrupted console.log(element + ", " +
			// JSON.stringify(q) + ", textStatus " + textStatus + ", " +
			// errorThrown);
		},
	...
	// with the following more stable code we cannot click on suggestions
	// anylonger
...
function dataToHtml(data, query) {
	var element = "";
	if (data.name)
		element += "<div class='nameseg'>" + formatValue(data.name, query)
				+ "</div>";
	var addStr = "";
	if (data.postcode)
	...
	if (addStr)
		element += "<div class='cityseg'>" + formatValue(addStr, query)
				+ "</div>";
